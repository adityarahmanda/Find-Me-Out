...
public class FieldOfView : MonoBehaviour
{
    ...
    private WaitForSeconds WaitForDelay = new WaitForSeconds(.2f);
    ...
    private void FindVisibleTargets()
    {
        ...
        for (int i = 0; i < targetsInViewRadius.Length; i++)
        {
            ...
            if(Vector3.Angle(transform.forward, dirToTarget) < viewAngle / 2)
            {
                ...
                {
                    if(target.gameObject.tag == "Hider")
                    {
                        if(PlayerManager.instance.IsPlayerCaught(target)) break;

                        PlayerManager.instance.CatchPlayer(target);
                    }
                }
            }
        }
